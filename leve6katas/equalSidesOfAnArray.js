...
// the integers to the left of N is equal to the sum of the integers to the right of N. If there is no index that would make this happen, 
// return -1.
...
function findEvenIndex(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
       let left = arr.slice(0,i)[0]?arr.slice(0,i).reduce((acc,cur)=> acc+cur):0
       let right = arr.slice(i+1)[0]?arr.slice(i+1).reduce((acc,cur)=> acc+cur):0
        if ( left === right){
            result = i
        }
    }
